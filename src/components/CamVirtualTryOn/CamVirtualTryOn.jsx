...
const CamVirtualTryOn = () => {
    ...
    const shoulderWidthHistory = useRef([]);
    const neckYHistory = useRef([]);
    const historySize = 5;
    ...
    const getAverageFromHistory = (currentValue, historyRef) => {
    ...
    const calculateClothPosition = (keypoints, canvas) => {
        ...
        // Calculate raw shoulder width and neck position
        const rawShoulderWidth = Math.abs(mirroredRightShoulderX - mirroredLeftShoulderX);
        const rawNeckY = nose.y + (Math.min(leftShoulder.y, rightShoulder.y) - nose.y) * 0.4;
        
        // Use averaged values for stability
        const shoulderWidth = getAverageFromHistory(rawShoulderWidth, shoulderWidthHistory);
        const neckY = getAverageFromHistory(rawNeckY, neckYHistory);
        ...
        if (leftHip && rightHip) {
            ...
            torsoHeight = shoulderWidth * 2.5;
        }

        // Calculate arm span for better width estimation
        let armSpan = shoulderWidth;
        if (leftElbow && rightElbow) {
            const leftArmWidth = Math.abs(leftElbow.x - leftShoulder.x) * 1.2;
            const rightArmWidth = Math.abs(rightElbow.x - rightShoulder.x) * 1.2;
            armSpan = shoulderWidth + leftArmWidth + rightArmWidth;
        } else if (leftWrist && rightWrist) {
            const leftArmWidth = Math.abs(leftWrist.x - leftShoulder.x) * 0.8;
            const rightArmWidth = Math.abs(rightWrist.x - rightShoulder.x) * 0.8;
            armSpan = shoulderWidth + leftArmWidth + rightArmWidth;
        }

        // Calculate cloth dimensions with dynamic scaling
        const clothWidth = armSpan * 1.5;
        ...
        const clothY = neckY - clothHeight * 0.2;

        // Calculate shoulder endpoints for perfect alignment
        const leftShoulderEnd = mirroredLeftShoulderX - (clothWidth * 0.15);
        const rightShoulderEnd = mirroredRightShoulderX + (clothWidth * 0.15);
            ...
            if (processedCloth && isTryOnActive) {
                try {
                    ...
                    if (poses && poses.length > 0 && poses[0].keypoints) {
                        ...
                        if (clothPosition) {
                            // Draw the clothing image with perfect alignment
                            ...
                            // Draw calculated neck position
                            ctx.fillStyle = 'yellow';
                            ctx.fillRect(
                                canvas.width / 2 - 5,
                                clothPosition.rawNeckY - 5,
                                10,
                                10
                            );
                            
                            // Draw cloth neck position
                            ctx.fillStyle = 'green';
                            ctx.fillRect(
                                clothPosition.x + clothPosition.width / 2 - 5,
                                clothPosition.y + clothPosition.height * 0.2 - 5,
                                10,
                                10
                            );
                        }
    ...
    return (
        <div className="cam-tryon-container">
            <button className="cam-tryon-dashboard-btn" onClick={() => navigate('/')}>
                Go to Dashboard
            </button>
            
            <h1 className="cam-tryon-title">Virtual Try-On</h1>
            
            <div className="cam-tryon-upload-section">
                <div className="cam-tryon-upload-content">
                    <div className="cam-tryon-upload-controls">
                        ...
                        <input
                            ...
                            className="cam-tryon-file-input"
                        />
                    </div>
                    
                    {processedCloth && (
                        <div className="cam-tryon-cloth-preview">
                            <p className="cam-tryon-preview-title">Cloth Preview</p>
                            <img
